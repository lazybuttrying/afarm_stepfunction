...
```bash
yolo$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
yolo$ sam local start-api
yolo$ curl http://localhost:3000/
```
...
```bash
yolo$ sam logs -n HelloWorldFunction --stack-name yolo --tail
```
...
```bash
yolo$ pip install pytest pytest-mock --user
yolo$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name yolo
```
