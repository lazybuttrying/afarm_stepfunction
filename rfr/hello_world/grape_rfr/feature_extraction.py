...
# 모든 변수들을 구하는!!!
# contour에서 이미지 외곽 잡는 경우 있으니 다 넘겨주면 ㅇ나됨
...
class Contours():
    ...
    def feature_extraction(self):
        ...
        df_result = pd.read_csv(self.dest_path)
        df_result = df_result.append({'image' : self.image_path , 'number of instances' : self.feat_num_instances, 'sunburn_ratio': self.sunburn_ratio , 'diameter' : self.feat_diameter, 'circularity':self.feat_circularity, 'density':self.density, 'aspect ratio': self.aspect_ratio,'average_hue':self.ave_hue, 'grade': self.grade} , ignore_index=True)
