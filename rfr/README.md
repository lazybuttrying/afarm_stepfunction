...
```bash
rfr$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rfr$ sam local start-api
rfr$ curl http://localhost:3000/
```
...
```bash
rfr$ sam logs -n HelloWorldFunction --stack-name rfr --tail
```
...
```bash
rfr$ pip install pytest pytest-mock --user
rfr$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name rfr
```
