...
# python track.py --yolo_weights ./yolov5/run22_best_yolov5.pt --source grape_demo.mp4 --save-txt --save-crop
# python track.py --yolo_weights ./yolov5/run22_best_yolov5.pt --source /data/home/genniferk/grape_ex.jpeg --save-txt --save-crop
...
def detect(opt):
    ...
    skip_frame = opt.skip_frame
    quality_id = opt.quality_id
    ...
    os.makedirs('/tmp/inference/crops/'+str(quality_id), exist_ok = True)
                ...
                if len(outputs) > 0:
                    for j, (output, conf) in enumerate(zip(outputs, confs)): 
                        ...
                        if save_crop:
                            ...
                            #######################
                            save_path = f'/tmp/inference/crops/{image_name}/{image_name}_{frame_idx}_{id}.png'
                            im1 = Image.fromarray(copy_im0)

                            if bbox_right > im1.size[0]-1 or bbox_bottom >im1.size[1]-1 or bbox_left>im1.size[0]-1 or bbox_top <0 or bbox_top>im1.size[1]-1:
                                print("size over")
                                continue
                            print("img size:", im1.size)
                            #print("bboxes",bbox_left,bbox_top,bbox_right,bbox_bottom)
                            try:
                                #crop_img = img_pil.crop((bbox_left,bbox_top,bbox_right,bbox_bottom)) 
                                crop_img = im1.crop((bbox_left,bbox_top,bbox_right,bbox_bottom))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default='/tmp/inference', help='output folder')  # output folder
    ...
    parser.add_argument("--skip_frame", type=int, default=5, help="how much frames to skip")
    parser.add_argument("--quality_id", type=int, help="how much frames to skip")
